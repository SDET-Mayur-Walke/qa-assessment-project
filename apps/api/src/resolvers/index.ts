...
import type { User, Product, ProductConnection, Cart, Order, AddToCartInput, ErrorCode } from "shared";
import { generateOrderNumber } from "shared";
import { seedData, getFeatureFlag, getChaosValue } from "../data/seed.js";
import { getUserFromAuth, storeLoginCode, verifyLoginCode, generateJWT } from "../services/auth.js";
import { 
  getSessionCart, 
  addItemToCart, 
...
async function applyResolverDelay(headers: any) {
...
export const resolvers = {
  ...
  Query: {
    ...
    products: async (_: any, args: { 
      query?: string; 
      first?: number; 
      after?: string; 
      tag?: string; 
    }, context: Context): Promise<ProductConnection> => {
      await applyResolverDelay(context.reply.request.headers);
      ...
      // Filter by tag  
      if (args.tag) {
        products = products.filter((p: Product) => p.tags.includes(args.tag!));
      }
      ...
      if (partialProductsBug && slice.length > 2) {
        ...
        if (edges[corruptIndex]) {
          edges[corruptIndex] = { node: null as any, cursor: edges[corruptIndex].cursor };
      ...
      if (errors.length > 0) {
        if (!(context.reply.request as any).mercurius) {
          (context.reply.request as any).mercurius = {};
        }
        (context.reply.request as any).mercurius.errors = errors;
      }
    ...
    product: async (_: any, args: { id?: string; slug?: string }, context: Context): Promise<Product | null> => {
      await applyResolverDelay(context.reply.request.headers);
    ...
    cart: async (_: any, __: any, context: Context): Promise<Cart> => {
      await applyResolverDelay(context.reply.request.headers);
      return getSessionCart(context.sessionId);
    },
    
    orders: async (_: any, __: any, context: Context): Promise<Order[]> => {
      const user = requireAuth(context.user);
      await applyResolverDelay(context.reply.request.headers);
      
      const userOrders = seedData.orders.filter((order: any) => order.userId === user.id);
  ...
  Mutation: {
    login: async (_: any, args: { email: string }, context: Context): Promise<boolean> => {
      await applyResolverDelay(context.reply.request.headers);
    ...
    confirmLogin: async (_: any, args: { email: string; code: string }, context: Context): Promise<string> => {
      await applyResolverDelay(context.reply.request.headers);
    ...
    addToCart: async (_: any, args: { input: AddToCartInput }, context: Context): Promise<Cart> => {
      await applyResolverDelay(context.reply.request.headers);
    ...
    updateCartItem: async (_: any, args: { productId: string; quantity: number }, context: Context): Promise<Cart> => {
    ...
    removeFromCart: async (_: any, args: { productId: string }, context: Context): Promise<Cart> => {
      await applyResolverDelay(context.reply.request.headers);
    ...
    applyCoupon: async (_: any, args: { code: string }, context: Context): Promise<Cart> => {
      await applyResolverDelay(context.reply.request.headers);
    ...
    checkout: async (_: any, __: any, context: Context): Promise<Order> => {
      const user = requireAuth(context.user);
      await applyResolverDelay(context.reply.request.headers);
